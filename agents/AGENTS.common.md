...
- **Merging**: repos following the `github/` merge-commit convention (see the root of this repo) reject a plain merge commit outright — default to `squash` (falling back to `rebase` if that's also disabled) rather than assuming `merge` will work.
